...
def safe_state(config):
    silent = config.dataset.quiet
    log_file = open(os.path.join(config.model.model_path, "log.txt"), "a")
    old_f = sys.stdout

    class F:
        def __init__(self, silent, old_f, log_file):
            self.silent = silent
            self.old_f = old_f
            self.log_file = log_file
            self.time_format = "%d/%m %H:%M:%S"

        def write(self, x):
            if not self.silent:
                if x.endswith("\n"):
                    x = f"{x[:-1]} [{datetime.now().strftime(self.time_format)}]\n"
                self.old_f.write(x)
    ...
    sys.stdout = F(silent, old_f, log_file)
